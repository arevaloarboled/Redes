...
$server.bind("192.168.250.37", 9123)
class GameWindow < Gosu::Window
  ...
  def setup_game
    begin
      ...
      #server.puts("hola")
      $server.send "hola",0,$servers[0],9123
      puts "lollol"
      #$server.close
  ...
  def update
    ...
    if @player #si existe jugador permite moverlo
      ...
      if Gosu::button_down? Gosu::KbLeft or Gosu::button_down? Gosu::GpLeft then
        #server = TCPSocket.open( $servers[0], 9123 )
        #$server.puts("left")
        $server.send "left",0,$servers[0],9123
        #server.close
        #@player.turn_left
      end
      if Gosu::button_down? Gosu::KbRight or Gosu::button_down? Gosu::GpRight then
        #server = TCPSocket.open( $servers[0], 9123 )
        #server.puts("right")
        $server.send "right",0,$servers[0],9123
        #server.close
        #@player.turn_right
      end
      if Gosu::button_down? Gosu::KbUp or Gosu::button_down? Gosu::GpUp then
        #server = TCPSocket.open( $servers[0], 9123 )
        #server.puts("up")
        $server.send "up",0,$servers[0],9123
        #server.close
        #@player.accelerate
      end
      ################--->>>
  ...
  def draw
    ...
    if @player #Si existe jugador lo dibuja
      ...
      #unless @player.lives <= 0 #Para que cuando muera no muestre mas en la pantalla
        #server = TCPSocket.open( $servers[0], 9123 )
        #server.puts("update")
        $server.send "update",0,$servers[0],9123
