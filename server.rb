...
def broadcast()
	ts=Thread.new do
		...
		loop{
			...
			if !$clients.include?(addr[3]) and data!='server'
				tmp=data.split(" ")
				$clients.push(tmp[1])
...
def server
	loop{
		begin
			if !$clients.empty?
				s = TCPSocket.open($clients[0], 9123)
				loop{
					data, addr = s.recvfrom(1024) # if this number is too low it will drop the larger packets and never give them to you #=> ["uuuu", ["AF_INET", 33230, "localhost", "127.0.0.1"]]
					puts "%s"[data]
				}
				s.close  
			end
